...
Usage:
  npx creative-skills list
  npx creative-skills add <skill-name> [--claude | --agent] [--global]
  npx creative-skills add-all [--claude | --agent] [--global]
...
if (command === 'list') {
    if (availableSkills.length === 0) {
        console.log('\nNo skills available in this package yet.\n');
        process.exit(0);
    }
...
if (command === 'add' || command === 'install') {
    ...
    if (availableSkills.length === 0) {
        console.log('\nNo skills available to install yet.\n');
        process.exit(0);
    }

    // No skill name given → install everything
    if (!skillName || skillName.startsWith('--')) {
        console.log(`\nInstalling all ${availableSkills.length} creative skills...\n`);
